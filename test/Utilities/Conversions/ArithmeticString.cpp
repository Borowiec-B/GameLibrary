...
TEST_CASE("arithmeticOrStringCast() sticks to guidelines (calls correct casting functions).")
{
	...
	REQUIRE(arithmeticOrStringCast<std::wstring>(std::numeric_limits<float>::infinity()) == toString<std::wstring>(std::numeric_limits<float>::infinity()));
	...
	REQUIRE(arithmeticOrStringCast<long double>(toWStringLDMax) == fromWStringLDMax);
